...
Viber ────┘       │                         ├─ Box / Dropbox / pCloud / … (rclone)
                  │                         └─ Локальная папка / NAS
                  ├──→ фото остальным в чат  (общая лента)
                  ↓
          удаление из чата
      (и ленты — только после проверки копии)
```
...
- **Очистка чата.** Снимок удаляется из переписки после успешной загрузки —
  и только после неё.
- **Общая лента.** Как только снимок лёг в облако, остальные видят его у себя
  в чате — **картинкой, а не файлом**, с именем отправителя. Из чатов он
  пропадает вместе с оригиналом, по тем же правилам очистки, а в облако второй
  раз не попадает: показывается копия, архивируется один файл.
...
Общая лента работает сразу: каждый сохранённый снимок показывается остальным
участникам. Выключается одной кнопкой — `/admin` → **👨‍👩‍👧 Семья** →
**Общая лента фото**.
