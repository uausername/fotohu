...
> Этот способ требует публичного HTTPS-адреса — он нужен для страницы
> возврата OAuth. **Если домена нет, подключите то же самое облако через
> [rclone](setup-rclone.md)**: он авторизуется локальным браузером, и
> публичный адрес не нужен вовсе.
