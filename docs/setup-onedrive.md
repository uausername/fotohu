...
## Автодобавление в альбом

Каждое новое фото и видео можно сразу добавлять в выбранный альбом OneDrive —
тот же, что показывается в приложении «Фото» (`/admin` → **🖼 Альбом OneDrive**).

- Работает **только для личного (Personal) аккаунта**. Альбомы в OneDrive —
  это отдельный ресурс Microsoft Graph (`bundle` с фасетом `album`), и для
  рабочего/учебного OneDrive for Business он не существует вовсе.
- Работает независимо от того, как подключена сама загрузка файлов (напрямую
  или через rclone) — это отдельная, минимальная авторизация тем же
  приложением из `ONEDRIVE_CLIENT_ID`, полученная через вход по коду на
  <https://microsoft.com/devicelogin>. Публичный домен для неё не нужен.
- Подключается один раз: **🔗 Подключить** → бот присылает код → ввести его на
  странице входа. Затем выбрать существующий альбом или создать новый.
- В альбом добавляются только фото и видео — документы и прочие файлы
  пропускаются.
